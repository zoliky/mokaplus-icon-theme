...
#              directories within the 'MokaPlus' folder. It ensures that any
...
#
# NOTES:       - The script expects 'MokaPlus' directory to be located
#                one folder above where this script resides.
#
################################################################################
...
# Resolve the path to the "MokaPlus" directory
mokaplus_dir=$(realpath "$(dirname "$(readlink -f "$0")")/../MokaPlus")

if [ ! -d "$mokaplus_dir" ]; then
  echo "Error: The 'MokaPlus' directory does not exist or is not accessible."
  exit 1
fi
...
# Find differences between sorted unique symlinks in $symlinks_file and actual symlinks in $mokaplus_dir
# For more information, see 'man comm'
script_dir="$(dirname "$(realpath "$0")")"

sort -u "$symlinks_file" > "$script_dir/symlinks_sorted.tmp"

find "$mokaplus_dir" -type l | while read -r symlink; do
...
echo "$comm_output" | while IFS= read -r line; do
  ...
  case "$line" in
    [[:space:]]*)  # Match if the line starts with spaces (indentation)
      ...
      destination=${line##* }
      # Iterate through each subdirectory in $mokaplus_dir and remove symlink
      for subdir in "$mokaplus_dir"/*/*; do
...
echo "$comm_output" | while IFS= read -r line; do
  ...
  case "$line" in
    ...
    *)  # This matches lines that are not indented (add action)
      ...
      source=${line%% *}
      destination=${line##* }
      # Iterate through each subdirectory in $mokaplus_dir and create symlink
      for subdir in "$mokaplus_dir"/*/*; do
